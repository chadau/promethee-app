    ...
    return (
        ...
        >
            <div className="flex gap-1 mt-2">
                {[...Array(8)].map((_, i) => {
                    const filledSegments = Math.round((battery / 100) * 8);
                    const isActive = i < filledSegments;
                    return (
                        <div
                            key={i}
                            className={`h-3 flex-1 rounded-sm transition-all duration-300 ${isActive
                                    ? 'bg-solar-amber shadow-[0_0_5px_#FFB347]'
                                    : 'bg-white/10'
                                }`}
                        />
                    );
